...
var (
	...
	frontFQDNs      = flag.String("server.frontfqdns", "", "YAML string representing a map from the name of each front provider to a FQDN that will reach this particular server via that provider (e.g. '{cloudflare: fl-001.getiantem.org, cloudfront: blablabla.cloudfront.net}')")
	reportingPeriod = flag.Int("stats.reportingperiod", 0, "time in seconds to wait between reporting stats. If not specified, stats are not reported. If specified, statshub, instanceid and statshubAddr must also be specified.")
	statshubAddr    = flag.String("stats.statshub", "pure-journey-3547.herokuapp.com", "address of statshub server")
	...
	proxyAll        = flag.Bool("client.proxyall", false, "set to true to proxy all traffic through Lantern network")
)
...
func (updated *Config) applyFlags() error {
	...
	flag.Visit(func(f *flag.Flag) {
		...
		case "instanceid":
			...
			// Stats
		case "stats.reportingperiod":
			updated.Stats.ReportingPeriod = time.Duration(*reportingPeriod) * time.Second
		case "stats.statshubaddr":
